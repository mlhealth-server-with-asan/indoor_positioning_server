...
public class PositionService {
    ...
    public WifiData addPosData(PosDataDTO posData) throws InvalidPasswordException {
        ...
        if (passwordEncoder.matches(posData.getPassword(), storedHash)) {
            ...
            System.out.println("wifiDataJson = " + wifiDataJson);
            wifiDataEntity.setWifiData(wifiDataJson);
            return wifiDataRepository.save(wifiDataEntity);
        } else {
...
//                        System.out.println("calPos" + "Matching BSSID found: " + sum);
                        break;
                    }
                }
            }
            ...
            //  System.out.println("dbData.getId() +\" \" +dbData.getPosition()+\" \"+count+\" \"+ratio = " + dbData.getId() +" " +dbData.getPosition()+" "+count+" "+ratio);
//            System.out.println("largestCount = " + largestCount + "count = " + count);
    ...
    public ResultDataDTO calcKnn(List<ResultDataDTO> results, int k, String android_id) {
        ...
        if (bestPosition != null) {
            return new ResultDataDTO(0, bestPosition, maxVotes, Double.MAX_VALUE, Double.MAX_VALUE);
        }
...
//    public ResultDataDTO calcKnn(List<ResultDataDTO> results, int k, String android_id) {
//        Map<String, List<ResultDataDTO>> groupedResults = results.stream()
...
//            System.out.println("bestResults = " + bestResults);
//            ResultDataDTO bestResult = bestResults.get(0);
//            //addBestResultToJsonArray(bestResult, 1, android_id); // bestResult를 JSON 배열에 추가
//
//            return bestResult;
//        }
//        return new ResultDataDTO(0, "not found", 0, Double.MAX_VALUE, Double.MAX_VALUE);
